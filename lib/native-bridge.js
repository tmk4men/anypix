...
(function(){
   ...
   * ・初回起動時に ATT（トラッキング許可）ダイアログを表示 → その後 AdMob を初期化。
  ...
  if(AdMob){
     ...
     * iOS はアプリが「アクティブ」になる前に requestTrackingAuthorization を呼ぶと、
     * ダイアログを出さずに即 denied を返す。この取りこぼしが起きると審査からは
     * 「ATT を実装していない」に見える（App Store ガイドライン 5.1.2(i) の指摘）。
     * → WebView が可視になってから明示的に要求する。
     */
    ...
    // 戻り値 true = ATT をこちらで処理済み（initialize 側の要求は不要）
    function requestATT(){
      if(typeof AdMob.trackingAuthorizationStatus !== 'function' ||
         typeof AdMob.requestTrackingAuthorization !== 'function'){
        return Promise.resolve(false);            // 旧版など → initialize に任せる
      }
      return AdMob.trackingAuthorizationStatus().then(function(res){
        // 'authorized' / 'denied' / 'restricted' は確定済み（OSは再表示しない）
        if(!res || res.status !== 'notDetermined') return true;
        return AdMob.requestTrackingAuthorization().then(function(){ return true; });
      }).catch(function(){ return false; });
    }

    // ATT → 初期化（失敗しても保存フローは止めない）
    var adsReady = whenVisible()
      .then(function(){ return delay(500); })     // アプリが active になるのを待つ
      .then(requestATT)
      .then(function(handled){
        return AdMob.initialize({ requestTrackingAuthorization: !handled });
      })
      .catch(function(){});
