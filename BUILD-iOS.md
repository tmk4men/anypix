...
3. **リワード広告（実装済み）** — `@capacitor-community/admob` で `window.AnyPixAds.showRewarded()` を注入。
   ...
   - **UMP のバージョン固定が必須**（`scripts/patch-ios-podfile.js`／`ios:setup`・`ios:sync` に組み込み済み）。admob 6.x は GMA を `11.3.0` に厳密固定する一方、UMP(GoogleUserMessagingPlatform) は `>= 2.1` の緩い依存で解決されるため、放置すると **UMP 3.x** が入り、`UMPConsentInformation`→`ConsentInformation` 等のリネームでプラグインの `ConsentExecutor.swift` がコンパイルエラー → **Archive 失敗**。Podfile に `pod 'GoogleUserMessagingPlatform', '< 3.0'` を入れて回避する。Capacitor 7 + admob 7.x（GMA 12.x/UMP 3.x）へ上げたらこの固定は削除。
